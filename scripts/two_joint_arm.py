...
class TwoJointArm:
    def __init__(self, k, L1=1.0, L2=1.0, order=3, reward_radius=0.25):
        ...
        # CONSTANT-DIFFICULTY task: targets are continuous points in the reachable workspace
        # (rejection-sampled so >=1 config lands within reward_radius). This decouples the target
        # set from the joint grid, so the fraction of "correct" actions is ~constant across k
        # (isolating the action-selection axis; no grid-alignment lumpiness -> no k=24 dip).
        self.eval_targets = self._make_reachable_targets(n=600, seed=12345)  # fixed eval set
        self.target = self.eval_targets[0]
        self.target_idx = 0

    def _reachable(self, xy):
        return bool(np.min(np.linalg.norm(self.fingertips - xy, axis=1)) <= self.reward_radius)

    def _rand_reachable(self, rng):
        while True:
            t1, t2 = rng.uniform(0, 2 * np.pi, 2)
            xy = np.array([self.L1 * math.cos(t1) + self.L2 * math.cos(t1 + t2),
                           self.L1 * math.sin(t1) + self.L2 * math.sin(t1 + t2)])
            if self._reachable(xy):
                return xy

    def _make_reachable_targets(self, n, seed):
        rng = np.random.RandomState(seed)
        return np.array([self._rand_reachable(rng) for _ in range(n)])
    ...
    def sample_target(self, idx=None):
        # idx None -> fresh continuous reachable target (training); else the fixed eval target idx
        if idx is None:
            self.target = self._rand_reachable(np.random)
        else:
            self.target_idx = idx
            self.target = self.eval_targets[idx]
        return self.target
